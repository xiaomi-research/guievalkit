...
""".strip(),
            ...
            "button": {
                "description": ("Back means returning to the previous interface, Home means returning to the desktop, "
                                "Menu means opening the application background menu, and Enter means pressing "
                                "the enter. Required only by `action=system_button`"),
                "enum": [
                    "Back",
                    "Home",
                    "Menu",
...
class ComputerUse(BaseTool):
    ...
    def description(self):
        return (
            ...
            "try adjusting your cursor position so that the tip of the cursor visually falls on the element that "
            "you want to click.\n"
            "* Make sure to click any buttons, links, icons, etc with the cursor tip in the center of the element. "
            "Don't click boxes on their edges unless asked.\n"
        ).strip()
...
""".strip(),
            ...
            "coordinate": {
                "description": ("(x, y): The x (pixels from the left edge) and y (pixels from the top edge) "
                                "coordinates to move the mouse to. Required only by `action=mouse_move` and "
                                "`action=left_click_drag`."),
                "type": "array",
            },
            "pixels": {
                "description": ("The amount of scrolling to perform. Positive values scroll up, negative "
                                "values scroll down. Required only by `action=scroll`."),
                "type": "number",
            },
            "time": {
