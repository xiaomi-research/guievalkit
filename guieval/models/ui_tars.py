...
from guieval.models.resources.ui_tars.instruction_message_builder import build as build_instruction_message


# section struct
logger = logging.getLogger(__name__)

ACTIONS = {"click", "long_press", "type", "scroll", "press_back", "press_home", "wait", "finished", "open_app"}
FIELDS = {"start_box", "end_box", "time", "direction", "content", "app_name"}
...
class UITars(ABCModel):
    NAMES = ("ui-tars-2b-sft", "ui-tars-7b-sft", "ui-tars-7b-dpo", "ui-tars-72b-sft", "ui-tars-72b-dpo")
    ...
    _DEFAULT_SAMPLING_PARAMS = SamplingParams(
        max_tokens=2048,
        temperature=0.1)
    DEFAULT_SAMPLING_PARAMS = dict.fromkeys(NAMES, _DEFAULT_SAMPLING_PARAMS)
    ...
    def parse_response(self, parsed_matches):
        ...
        try:
            ...
            if 'press' not in action and action not in {'wait', 'finished'} and not arguments:
    ...
    def model_2_minicpm(self, output_text, width, height) -> MINICPM_ACTION:
        ...
        if parsed_response['action'] == "click":
            ...
            return {'action': 'SCROLL',
                    'arguments': {"POINT": (500, 500), "to": reverse_map.get(direction)}}
    ...
    def aitw_2_model_action(step_task: StepTaskModel, resized_height: int, resized_width: int) -> MODEL_ACTION:
        ...
        if action_type == ActionType.DUAL_POINT:
            ...
            return {'action': 'long_press',
                    'arguments': dict(start_box=f'({box[0]},{box[1]})', time=1000)}
    ...
    def model_2_contents(self, history_step_task, current_step_task, action, *,
        ...
        # history content required by ui-tars is the original answer message
        ...
        if action['action'] == 'click':
            ...
            return {'content': self._wrap_answer(answer=(('click(start_box=\'<|box_start|>'
                                                          f'({box[0]},{box[1]})<|box_end|>\')')
                                                         if box else
                                                         'click(start_box=\'<|box_start|>(-1, -1)<|box_end|>\')'),
            ...
            box = action['arguments'].get('start_box')
            return {'content': self._wrap_answer(answer=(('long_press(start_box=\'<|box_start|>'
                                                          f'({box[0]},{box[1]})<|box_end|>\')')
                                                         if box else
                                                         'long_press(start_box=\'<|box_start|>(-1, -1)<|box_end|>\')'),
                                                 thought=history_step_task.low_instruction,
                                                 enable_think=current_step_task.enable_think),
                    'source': 'low_instruction' if history_step_task.low_instruction else 'offline_rule'}
            ...
            return {'content': self._wrap_answer(answer=(f'scroll(direction=\'{direction}\')'
                                                         if start_box and direction else
                                                         'scroll(direction=\'unknown\')'),
                                                 thought=history_step_task.low_instruction,
                                                 enable_think=current_step_task.enable_think),
                    'source': 'low_instruction' if history_step_task.low_instruction else 'offline_rule'}
            ...
            return {'content': self._wrap_answer(answer=f'finished()',
                                                 thought=history_step_task.low_instruction,
                                                 enable_think=current_step_task.enable_think),
                    'source': 'low_instruction' if history_step_task.low_instruction else 'offline_rule'}
    ...
    def prepare_task_input(self, step_task: StepTaskModel, image_memory: int = 5, **kwargs) -> StepTaskModel:
        ...
        truncated_history_indices = filtered_history_indices[-(image_memory - 1):]
